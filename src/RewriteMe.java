...
public class RewriteMe {
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        List<String> tempList = questions.stream()
                .flatMap(e -> e.getAllAnswers().stream())
                .filter(e -> e.equalsIgnoreCase(answerCandidate))
                .collect(Collectors.toList());
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        return questions.stream()
                .collect(groupingBy(Question::getCategory, Collectors.mapping(Question::getQuestionString,
                        Collectors.toList())));
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c) {
       return questions.stream()
                ...
                .flatMap(e -> e.getAllAnswers().stream())
                .reduce(" ", (acc, e) -> {if(acc.length()<e.length())return e; else return acc;});
    }
